...
class DialogAgent(Agent):
    ...
    @lab_api
    def act(self, obs):# step 2
        '''Standard act method from algorithm.'''
        action = self.algorithm.act(self.body.encoded_state)
        print(self.body.encoded_state)
        ...
        logger.nl(f'System action: {action}')
        logger.nl(f'System utterance: {decoded_action}')

        return decoded_action

    def state_update(self, obs, action): # step 1
        # update history 
        if self.dst:
        ...
        if isinstance(self.dst, word_dst.SUMBTTracker):  # for SUMBT
            input_act = da_normalize(state['user_action'], role='usr')
        ...
        elif isinstance(self.dst, word_dst.SUMBTTracker):  # for SUMBT
            input_act = state['user_action']
            #self.dst.state['user_action'] = input_act
    ...
    def action_decode(self, action, state): # step 3
    ...
    @lab_api
    def update(self, obs, action, reward, next_obs, done): # step 4
        '''Update per timestep after env transitions, e.g. memory, algorithm, update agent params, train net'''
        # update state
