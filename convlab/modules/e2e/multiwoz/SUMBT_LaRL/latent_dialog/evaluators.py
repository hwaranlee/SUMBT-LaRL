...
from convlab.modules.word_policy.multiwoz.larl.latent_dialog.normalizer import delexicalize as delex
...
from convlab.modules.e2e.multiwoz.SUMBT_LaRL.utils.db_utils import query
...
class MultiWozDB(object):
    ...
    for domain in domains:
        db = 'data/norm-multi-woz/db/{}-dbase.db'.format(domain)
...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def __init__(self, data_name, delex_data_dir):
        self.data_name = data_name
        self.delex_dialogues = json.load(open(delex_data_dir, 'r'))
        self.labels = list()
        self.hyps = list()
    ...
    def _parseGoal_convai(self, goal, d, domain, learning_step):
        train_reqt_slots = ['trainID']
        if learning_step == 1: 
            train_reqt_slots.extend(['leaveAt', 'arriveBy'])
        elif learning_step == 2:
            train_reqt_slots.extend(['leaveAt', 'arriveBy', 'price', 'duration'])
            ...
            if domain == 'train':
                ...
                if 'reqt' in d['goal'][domain]:
                    for s in d['goal'][domain]['reqt']:
                        if s in train_reqt_slots:
                            if s == 'trainID':
                                goal[domain]['requestable'].append('trainid')
                            elif s == 'leaveAt':
                                goal[domain]['requestable'].append('leaveat')
                            elif s == 'arriveBy':
                                goal[domain]['requestable'].append('arriveby')
                            else:
                                goal[domain]['requestable'].append(s)
                ...
                if 'reqt' in d['goal'][domain]:
                    for s in d['goal'][domain]['reqt']:  # addtional requests:
                        if s in ['phone', 'address', 'postcode']:
                            # ones that can be easily delexicalized
                            goal[domain]['requestable'].append(s)

                    if learning_step == 1 or learning_step == 2:
                        if domain == 'attraction' and 'type' in d['goal'][domain]['reqt']:
                            goal[domain]['requestable'].append('type')
                        if domain == 'taxi' and 'car type' in d['goal'][domain]['reqt']:
                            goal[domain]['requestable'].append('type')
                    
                    if learning_step == 2:
                        if domain == 'restaurant' and 'food' in d['goal'][domain]['reqt']:
                            goal[domain]['requestable'].append('food')
                        if domain == 'hotel' and 'stars' in d['goal'][domain]['reqt']:
                            goal[domain]['requestable'].append('stars')
                        if domain == 'attraction' and 'entrance fee' in d['goal'][domain]['reqt']:
                            goal[domain]['requestable'].append('entrance')
    ...
    def _evaluateGeneratedDialogue_convai(self, dialog, realDialogue, learning_step=0, soft_acc=False, use_oracle_bs=False):
        domains = ['restaurant', 'hotel', 'attraction', 'train', 'taxi', 'hospital', 'police']
        requestables = ['phone', 'address', 'postcode', 'reference', 'trainid']
        requestables_train = []
        requestables_attraction = []
        if learning_step == 1: 
            requestables_train.extend(['leaveAt', 'arriveBy'])
            requestables_attraction.extend(['type'])
        elif learning_step == 2:
            requestables_train.extend(['leaveAt', 'arriveBy', 'price', 'duration'])
            requestables_attraction.extend(['type', 'entrance'])
        ...
        for domain in domains:
            if realDialogue['goal'][domain]:
                goal = self._parseGoal_convai(goal, realDialogue, domain, learning_step)
        ...
        for t, (sent_t, state_t) in enumerate(dialog):
        #for t, sent_t in enumerate(dialog):
            for domain in goal.keys():
                # for computing success
                if '[' + domain + '_name]' in sent_t or '_trainid' in sent_t:
                    if domain in ['restaurant', 'hotel', 'attraction', 'train']:
                        # HERE YOU CAN PUT YOUR BELIEF STATE ESTIMATION
                        if use_oracle_bs:
                            venues, _ = query(domain, realDialogue['log'][t * 2 + 1]['metadata'][domain]['semi'])
                        else:
                            venues, _ = query(domain, state_t[domain]['semi'])
                        ...
                        if domain + '_reference' in sent_t:
                            if use_oracle_bs:
                                if 'restaurant_reference' in sent_t:
                                    if realDialogue['log'][t * 2 + 1]['db'][-3] == 1:  # if pointer was allowing for that?
                                        provided_requestables[domain].append('reference')
                                elif 'hotel_reference' in sent_t:
                                    if realDialogue['log'][t * 2 + 1]['db'][-2] == 1:  # if pointer was allowing for that?
                                        provided_requestables[domain].append('reference')
                                elif 'train_reference' in sent_t:
                                    if realDialogue['log'][t * 2 + 1]['db'][-1] == 1:  # if pointer was allowing for that?
                                        provided_requestables[domain].append('reference')
                                else:
                                    provided_requestables[domain].append('reference')
                            else:
                                if 'restaurant_reference' in sent_t:
                                    if state_t['restaurant']['book']['people'] not in ['', 'none', 'not mentioned', 'dontcare'] and state_t['restaurant']['book']['day'] not in ['', 'none', 'not mentioned', 'dontcare'] and state_t['restaurant']['book']['time'] not in ['', 'none', 'not mentioned', 'dontcare']:
                                        provided_requestables[domain].append('reference')
                                elif 'hotel_reference' in sent_t:
                                    if state_t['hotel']['book']['people'] not in ['', 'none', 'not mentioned', 'dontcare'] and state_t['hotel']['book']['day'] not in ['', 'none', 'not mentioned', 'dontcare'] and state_t['hotel']['book']['stay'] not in ['', 'none', 'not mentioned', 'dontcare']:
                                        provided_requestables[domain].append('reference')
                                elif 'train_reference' in sent_t:
                                    if state_t['train']['book']['people'] not in ['', 'none', 'not mentioned', 'dontcare']:
                                        provided_requestables[domain].append('reference')
                                else:
                                    provided_requestables[domain].append('reference')
                    else:
                ...
                if domain == 'train':
                    for requestable in requestables_train:
                        if domain + '_' + requestable + ']' in sent_t:
                            provided_requestables[domain].append(requestable)
                
                if domain == 'attraction':
                    for requestable in requestables_attraction:
                        if domain + '_' + requestable + ']' in sent_t:
                            provided_requestables[domain].append(requestable)

                if learning_step == 1 or learning_step == 2:
                    if domain == 'taxi' and 'taxi_type' in sent_t:
                        provided_requestables[domain].append('type')
                
                if learning_step == 2:
                    if domain == 'restaurant' and 'restaurant_food' in sent_t:
                        provided_requestables[domain].append('food')                    
                    if domain == 'hotel' and 'hotel_stars' in sent_t:
                        provided_requestables[domain].append('stars')
        ...
        for domain in goal.keys():
            ...
            if domain == 'train':
                if not venue_offered[domain]:
                    if 'reqt' in realDialogue['goal'][domain] and 'trainID' not in realDialogue['goal'][domain]['reqt']:
                        venue_offered[domain] = '[' + domain + '_name]'
        ...
        for domain in goal.keys():
            match_stat = 0
            if domain in ['restaurant', 'hotel', 'attraction', 'train']:
                goal_venues, _ = query(domain, goal[domain]['informable'])
        ...
        verbose = False
        if verbose and ((match < 1.0) or (match == 1.0 and success < 1)):
            pdb.set_trace()
            print(goal.keys())
            print(stats)
            ...
            for i, turn in enumerate(dialog):
                print('%d | user: %s' % (i, realDialogue['log'][2*i]['text']))
                print('%d | sys : %s' % (i, realDialogue['log'][2*i+1]['text']))
                print('%d | sys : %s' % (i, realDialogue['log'][2*i+1]['text_delex']))
                print('%d | pred: %s '% (i, turn[0]))
                print('-----')
    ...
    def evaluateModel(self, dialogues, learning_step, mode='valid', reduce='mean', use_oracle_bs=False):
        ...
        for filename, dial in dialogues.items():
            if mode == 'rollout':
                ...
                '''
                success, match, stats = self._evaluateGeneratedDialogue_convai(dial, data, learning_step, soft_acc=mode =='offline_rl', use_oracle_bs=use_oracle_bs)
        ...
        mean_successes = np.sum(successes)/float(total)
        mean_matches = np.sum(matches)/float(total)
        
        report = ""
        report += '{} Corpus Matches : {:2.2f}%'.format(mode, ( mean_matches * 100)) + "\n"
        report += '{} Corpus Success : {:2.2f}%'.format(mode, ( mean_successes * 100)) + "\n"
        report += 'Total number of dialogues: %s \n' % total
        
        report += 'Statistics: \n'
        for domain in gen_stats.keys():
            report += domain + ' '+ ' '.join([str(x) for x in gen_stats[domain]]) + '\n'
        report += 'Statistics (SNG): \n'
        for domain in sng_gen_stats.keys():
            report += domain + ' ' + ' '.join([str(x) for x in sng_gen_stats[domain]]) + '\n'

        if reduce == 'mean':
            return report, mean_successes, mean_matches
        elif reduce == 'none':
            return report, successes, matches
