...
class SysPerfectBD2Cat(BaseModel):
    def __init__(self, config, processor, device):
        ...
        self.bs_size = processor.bs_size
        ...
        self.c2z = nn_lib.Hidden2Discrete(self.utt_emb_size + self.db_size + self.bs_size,
        ...
        if not self.simple_posterior:
            if self.contextual_posterior:
                self.xc2z = nn_lib.Hidden2Discrete(self.utt_emb_size + self.utt_encoder.output_size + self.db_size + self.bs_size,
                                                   config.y_size, config.k_size, is_lstm=False)
            else:
    ...
    def forward(self, data_feed, mode, clf=False, gen_type='greedy', use_py=None, return_latent=False):
        ...
        enc_last = th.cat([bs_label, db_label, utt_summary], dim=1)
