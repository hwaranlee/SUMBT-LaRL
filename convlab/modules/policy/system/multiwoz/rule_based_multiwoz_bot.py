...
class RuleBasedMultiwozBot(SysPolicy):
    ...
    def _update_DA(self, user_act, user_action, state, DA):
        ...
        if intent_type == 'Request':
            ...
            # There are multiple results matching user's constraint
            else:
                p = random.random()
    ...
    def _update_train(self, user_act, user_action, state, DA):
        ...
        if user_act == 'Train-Request':
            ...
            for slot in user_action[user_act]:
                ...
                slot = slot[0].capitalize()
                slot_name = REF_SYS_DA['Train'].get(slot,slot)

                try:
                    DA['Train-Inform'].append([REF_USR_DA['Train'].get(slot, slot), kb_result[0][slot_name]])
                except:
                    pass
            return
        if len(kb_result) == 0:
