...
class BeliefTracker(nn.Module):
    ...
    def forward(self, input_ids, input_len, labels=None, n_gpu=1, target_slot=None):
        ...
        if self.zero_init_rnn:
            h = torch.zeros(self.rnn_num_layers, input_ids.shape[0] * num_req, self.hidden_dim).to(self.device)  # [1, slot_dim*ds, hidden]
        else:
        ...
        if labels is None:
            return output, hidden_req, hidden_gen
        ...
        if n_gpu == 1:
            return loss_bs, loss_slot, acc, acc_slot, pred_slot, loss_req, acc_req, pred_req, loss_gen, acc_gen, pred_gen
        else:
            return loss_bs.unsqueeze(0), None, acc.unsqueeze(0), acc_slot.unsqueeze(0), pred_slot.unsqueeze(0), \
                   loss_req.unsqueeze(0), acc_req.unsqueeze(0), loss_gen.unsqueeze(0), acc_gen.unsqueeze(0)
