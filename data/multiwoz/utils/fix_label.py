...
def normalize_value_name(value):
    value = value.lower().strip()

    if value == '' or value == 'not mentioned' or value == 'not given' or len(value) == 0:
        value = 'none'
...
def fix_general_label_error(label_dict): #, type, slots):
    ...
    GENERAL_TYPO = {
        ...
        "free internet":"yes",
        # star
        "4 star":"4", "4 stars":"4", "0 star rarting":"none", "4-star":"4", "four star":"4", "four stars":"4", "three":"3", "two":"2",
        # others 
    ...
    for slot in label_dict.keys():

        if slot == 'booked':
            continue

        label_dict[slot] = normalize_value_name(label_dict[slot])
